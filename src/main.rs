...
fn bowl(throws: &[i32], score: i32) -> i32 {
    if throws.len() <= 3 {
    ...
    match throws {
        [10, rest @ ..] => bowl(rest, score + 10 + rest[0] + rest[1]), // Strike
        [t1, t2, rest @ ..] if t1 + t2 == 10 => bowl(rest, score + 10 + rest[0]), // Spare
        [t1, t2, rest @ ..] => bowl(rest, score + t1 + t2),            // Open frame
        _ => panic!(),
    }
}
...
mod tests {
    ...
    fn bowl_test(#[case] expected: i32, #[case] input: &[i32]) {
        assert_eq!(expected, bowl(input, 0))
    }
}
